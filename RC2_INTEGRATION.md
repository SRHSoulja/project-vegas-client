# V1.7 rc.2 — operator integration candidate
...
```bash
mkdir -m 700 vegas-rc2-download
cd vegas-rc2-download
base=https://github.com/SRHSoulja/project-vegas-client/releases/download/v1.7.0-rc.2
...
```bash
curl --fail --location --proto '=https' "$base/vegas-1.7.0-rc.2-linux-amd64.tar.gz" -o vegas-1.7.0-rc.2-linux-amd64.tar.gz
...
```bash
tar -xzf vegas-1.7.0-rc.2-linux-amd64.tar.gz
pilot_home="$HOME/vegas-rc2-pilot"
mkdir -m 700 "$pilot_home"
env HOME="$pilot_home" python3 vegas-1.7.0-rc.2/verify-install.py vegas-1.7.0-rc.2 --anchor "$PWD/release.pub" --install
env HOME="$pilot_home" "$pilot_home/.local/lib/project-vegas/1704/bin/vegas"
```
...
## Small next human study — separate empty gas wallet

Do this only when you are ready for the next study; completing alpha.2 did not require it. **Funding: none. No token approval, deposit, transfer, exit, sponsor request or transaction is needed.**

After authentication and installation in the new isolated study home, you may leave the Vegas identity uninitialized for this gas-wallet-only test. The menu's identity-file line names where an identity would live; it is not required for these gas-wallet actions.

1. Choose12 to create an **empty separate test-gas wallet**. Choose and repeat its password privately. This is not your Vegas identity backup.
2. Choose13 and keep its public address for a local comparison. Do not send it.
3. Choose22 to export an encrypted gas-wallet backup. Use a full absolute filename in a separate private directory. Retain that backup and its password; do not send either.
4. Quit0, restart the same rc.2 study client with its isolated HOME, and choose13. Report only whether the address matches, whether the two types of backup are clearly distinguished, confusing labels and ordinary errors. Then quit.

This tests the empty local gas-wallet boundary, encrypted-backup flow and restart persistence. It does not demonstrate gas funding, a token transaction or human gas-wallet restoration. The no-overwrite restore path has separate automated evidence. Never fund this wallet as part of this study.
